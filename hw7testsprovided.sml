...
let
	val LineSegment(a,b,c,d) = preprocess_prog (LineSegment(3.2,4.1,3.5,~4.1))
	val LineSegment(e,f,g,h) = LineSegment(3.2,4.1,3.5,~4.1)
in
	if real_equal(a,e) andalso real_equal(b,f) andalso real_equal(c,g) andalso real_equal(d,h)
	then (print "preprocess maintains a proper LineSegment successfully\n")
	else (print "preprocess does not maintain a proper LineSegment successfully\n")
end;
...
(* eval_prog tests with Shift*)
val test1 = eval_prog (Shift(3.0,4.0, NoPoints), [])
